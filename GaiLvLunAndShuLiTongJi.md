...
### 1. 全概率公式；
### 2. 贝叶斯公式；
#### a. 贝叶斯思路：在真实世界中，我们 往往把现象的经验推理，和某种先验结合，去估计事务的可能性；
#### b. 贝叶斯的数学公式：
##### i. 需要有先验概率P(A)；
##### ii. 似然性P（B|A）；
##### iii. 最终得到后验概率P（A|B）;
##### iv. 这三者构成了贝叶斯统计的三要素，基于此的贝叶斯定律数学方程极为简单：
##### P(A│B)= (P(B│A)P(A))/(P(B))
#### c. 贝叶斯公式的通俗理解：
##### i. 贝叶斯公式是解决由观察到的现象（或是测量的数据）去推断现象（或是数据）后面的规律发生的概率问题。不妨将贝叶斯公式中的A换成“规律”，B换成“现象”，公式一目了然：
...
#### c. 则F(n, m) = X / n / Y/m；
### ７. 点估计：
#### a. 参数：反映总体某方面特征的量；比如：合格率，均值，方差；
#### b. 参数估计的形式：点估计和区间估计；
#### c. 点估计：
##### i. 总体X含有未知参数θ，  X1， X2， …., Xn是总体的简单随机样本；
##### ii. 构造合适的统计量θ= θ(X1, X2, …, Xn)用来估计未知参数：
##### 	1) 点估计量——前者；
##### 	2) 点估计值——后者；
#### d. 常用的点估计法：
##### i. 矩估计；
##### 	1) 前提：不需要直到总体服从的分布类型；
##### ii. 极大似然估计；
##### 	1) 前提：使用极大似然估计，需要知道总体服从某种确定的概率分布，只是参数未知；
##### 	2) 前提：总体X服从 p(x; θ)分布，θ未知，即每一个样本Xi出现的概率都和参数θ有关；
##### 	3) 似然函数；L(θ)= ∏24_(i=1)^n▒〖p(x_i;θ)〗—所有样本出现的概率乘积与θ有关；
##### 	4) 求参数θ使得似然函数最大；
