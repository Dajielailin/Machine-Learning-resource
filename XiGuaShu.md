...
## 16. 决策树如何应用到数值特征？
### a. 假设有一个数值特征，该特征有大量的唯一值，在构建树的每一步过程中，会有过多的二元属性可供选择，那么如何寻找具有最好分割的二元属性？——启发式算法来限制选择的属性数量；
### b. 决策树处理数值特征最简单的启发式算法是升序排列该特征的值，然后只关注那些是目标变量发生变化的特征值；
### c. 当数据集具有大量数值特征，且每个特征具有大量唯一值时，只选用具有增益的前N个值；
### 
## 17. 支持向量机：
...
### f. 半正定矩阵？
### g. 希尔伯特空间的特征空间？
### h. H为核函数K(,)对应的再生核希尔伯特空间，||h||H表示H空间中关于h的范数？
## 18. 核方法：
### a. 基于核函数发展出来的一些列学习方法；
### b. 最常见的是通过“核化”来将线性学习器拓展成为非线性学习器；——核线性判别分析（KLDA）；
### c. 类间散度矩阵？
### d. 类内散度矩阵？
### e. 核函数直接决定了支持向量机与核方法的最终性能，但遗憾的是，核函数的选择是一个未知问题——多核学习；
### f. 多核学习：即使用多个核函数并通过学习获得其最优凸组合最为最终的核函数；
### g. 常见的核函数：
#### i. 线性核：即向量的内积；
#### ii. 多项式核：线性核的多次方；
#### iii. 高斯核；
...
#### v. Sigmoid核；
## 19. 贝叶斯分类器：
### a. 贝叶斯决策论：概率框架下实施决策的基本方法；
#### i. 对于分类任务来说，已知先验概率，贝叶斯决策论考虑如何基于这些概率和误判损失来选择最优 的标记类别；
### b. 对于分类任务，基于后验概率P(ci | x)可获得将样本x分类为ci所产生的期望损失，即在样本x上的“条件风险”：
#### i. R(ci | x) = ∑24_(j=1)^N▒〖δ_ij P(c_j  ┤|x)〗
#### ii. 其中N为分类任务中可能的类别标记即 y = {c_1, c_2, 〖…., c〗_N}，δ_ij  是将一个真实标记为c_j  的样本误分类为 c_i 所产生的损失；
### c. 贝叶斯判定准则：
#### i. 为最小化总体风险，只需在每个样本上选择那个能使条件风险R(c | x)最小的类别标记：
##### 1) h∗ (x)=arg⁡min⁡█(R(c ┤|  x)@c ϵ y)         ——贝叶斯最优分类器
### d. 若目标是最小化分类错误率，则相应的贝叶斯最优分类器是：
#### i. h∗ (x)=   arg⁡max⁡〖 P(c ┤|x)    ；〗 
#### ii. 即对每个样本x，选择能使后验概率P(c | x)最大的类别标记；
#### iii. 相应地，机器学习要实现的是基于有限的训练样本尽可能地准确估计出后验概率P(c | x)；
#### iv. 由贝叶斯定理，可将上述问题转化为：如何基于训练数据D来估计先验概率P(c)和似然估计：
##### 1) 依据大数定律，类先验概率P(c)可通过个类样本出现地频率来估计；
##### 2) 对类条件概率P(x | c)，由于它涉及关于x所有属性的联合概率，很难获取未被观测到的样本，所有比较难估计：
##### 3) 估计类条件概率的方法：
###### a) 极大似然估计：先假定该分类任务具有某种确定的概率分布形式，再基于训练样本对概率分布的参数进行估计；
###### b) 朴素贝叶斯分类器：
####### i) 前提：假设分类问题中，所有属性相互独立；
####### ii) 表达式：hnb (x)=arg⁡max⁡〖P(c)  ∏24_(i=1)^d▒〖P(x_i  ┤|  c) 〗〗 ；
####### iii) 其中d为属性数目，x_i  为x 在第i个属性上的取值；
####### iv) 为了避免其他属性携带的信息被训练集中未出现的属性值(P(xi | c) = 0)“抹去”——拉普拉斯修正；
###### c) 对属性独立性假设进行放松：半朴素贝叶斯分类器
####### i) 基本想法：适当考虑一部分属性间的相互依赖信息：
####### ii) 常见策略：
####### 	One. 独依赖估计 ——ODE：即假设每个属性在类别之外最多仅依赖一个其他属性；
####### 		First. SPODE
### 					AODE